...
const LIPSTICK_SHADES = [
  { id: 0, name: "None", color: "transparent" },
  // Reds & Pinks
...
export default function VirtualTryOn() {
  ...
  const latestLandmarksRef = useRef(null); // Ref to hold the latest landmarks
  ...
  useEffect(() => {
    ...
    // Set up the onResults callback to store the latest landmarks
  ...
  // This function now starts the animation loop that handles both processing and drawing
  const startProcessing = () => {
    const processFrame = async () => {
        ...
        if (video && video.readyState >= 4) {
            ...
            // Drawing logic is now inside the animation frame loop for synchronization
            const canvas = canvasRef.current;
            ...
            ctx.translate(width, 0); // Flip horizontally for a mirror effect
            ctx.scale(-1, 1);
            ctx.drawImage(video, 0, 0, width, height);

            // Draw lipstick if landmarks are available
            if (latestLandmarksRef.current) {
                drawLipstick(ctx, latestLandmarksRef.current, width, height);
            }
            ctx.restore();
        }
        // Continue the loop
  ...
  const drawLipstick = (ctx, landmarks, w, h) => {
    ...
    for (let i = 1; i < upperOuterPts.length; i++) lipShape.lineTo(upperOuterPts[i].x, upperOuterPts[i].y);
    for (let i = lowerOuterPts.length - 1; i >= 0; i--) lipShape.lineTo(lowerOuterPts[i].x, lowerOuterPts[i].y);
    lipShape.closePath();

    const mouthOpening = new Path2D();
    mouthOpening.moveTo(upperInnerPts[0].x, upperInnerPts[0].y);
    for (let i = 1; i < upperInnerPts.length; i++) mouthOpening.lineTo(upperInnerPts[i].x, upperInnerPts[i].y);
    for (let i = lowerInnerPts.length - 1; i >= 0; i--) mouthOpening.lineTo(lowerInnerPts[i].x, lowerInnerPts[i].y);
        ...
        {snapshot && (
            <div className="absolute inset-0 bg-black/80 z-30 flex flex-col items-center justify-center p-4">
                ...
                <div className="mt-8 flex gap-4">
                    <button onClick={() => setSnapshot(null)} className="px-6 py-2 bg-gray-700 text-white rounded-full font-semibold hover:bg-gray-600 transition-colors">
                        Back
                    </button>
                    <a 
                        href={snapshot} 
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4 pb-8 sm:p-6 sm:pb-10 lg:p-8 lg:pb-12 bg-black/30 z-10">
            <div className="max-w-6xl mx-auto flex flex-col items-center gap-6">
                    ...
                    {LIPSTICK_SHADES.map((shade) => (
                      <div key={shade.id} className="relative flex flex-col items-center">
                          <button
                            ...
                            className={`w-8 h-8 rounded-full transition-transform duration-200 ease-in-out border border-white/30 ${selectedShade.id === shade.id ? 'scale-110' : ''}`}
                            style={{ backgroundColor: shade.color }}
                            title={shade.name}
                          >
                            {shade.id === 0 && (
                                <span className="text-white text-xs leading-none">None</span>
                            )}
                          </button>
                           <div className={`absolute -bottom-2.5 h-1 w-1 rounded-full bg-red-500 transition-opacity ${selectedShade.id === shade.id ? 'opacity-100' : 'opacity-0'}`}></div>
                      </div>
                    ))}
                </div>
                ...
                <div className="w-full max-w-sm flex items-center justify-around text-white">
                    ...
                    {/* Shutter Button */}
                    <button onClick={takeSnapshot} className="w-20 h-20 rounded-full bg-white p-1.5 shadow-lg active:scale-95 transition-transform">
